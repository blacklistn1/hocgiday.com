...
class Thanhnhac extends CI_Controller {
  function __construct()
  {
    parent::__construct();
    $this->load->model('m_thanhnhac');
  ...
  function city($city = '', $page = '1')
  {
    ...
    else
    {
      ...
      if ($this->form_validation->run() === FALSE)
      {
        ...
        $data['result'] = $this->m_thanhnhac->read_data($field ,$where, $cond);
        ...
        $data['title'] = 'Tìm giáo viên Thanh nhạc tại nhà hoặc tại lớp';
        $this->load->view('client/thanhnhac_hanoi', $data);
      }
      else
      {
  ...
  {
    $res = count($this->m_thanhnhac->read_data($field, $where));
